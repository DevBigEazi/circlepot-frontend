...
export const useCreditScore = () => {
    ...
    const [reputationDecreases, setReputationDecreases] = useState<ReputationDecrease[]>([]);
    const [categoryChanges, setCategoryChanges] = useState<ScoreCategoryChange[]>([]);
    const [circleCompletions, setCircleCompletions] = useState<CircleCompletion[]>([]);
    ...
    useEffect(() => {
        if (reputationData) {
            ...
            const increases = (reputationData.reputationIncreaseds || []).map((inc: any) => ({
                id: inc.id,
            ...
            const decreases = (reputationData.reputationDecreaseds || []).map((dec: any) => ({
                id: dec.id,
            ...
            const changes = (reputationData.scoreCategoryChangeds || []).map((change: any) => ({
                id: change.id,
            ...
            const circles = (reputationData.circleCompleteds || []).map((circle: any) => ({
                id: circle.id,
            ...
            const lates = (reputationData.latePaymentRecordeds || []).map((late: any) => ({
                id: late.id,
