...
export const useNotificationSync = (
    ...
    useEffect(() => {
        ...
        circles.forEach((circle) => {
            ...
            if (circle.isStarted && !processedEvents.current.has(`circle_started_${circleId}`)) {
                addNotification({
            ...
            if (circle.isCompleted && !processedEvents.current.has(`circle_completed_${circleId}`)) {
                addNotification({
    ...
    useEffect(() => {
        ...
        transactions.forEach((tx) => {
            ...
            if (tx.type === 'circle_payout' && !processedEvents.current.has(`payout_${txId}`)) {
                addNotification({
                    title: 'Payment Received',
                    message: `You received ${tx.amount} from "${tx.circleName || 'Circle'}" payout.`,
            ...
            if (tx.type === 'late_payment' && !processedEvents.current.has(`late_${txId}`)) {
                addNotification({
