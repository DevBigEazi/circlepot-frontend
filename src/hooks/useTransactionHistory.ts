    ...
    const {
        ...
        async queryFn() {
            ...
            try {
                ...
                // Fetch cUSD Transfer events using Thirdweb
                ...
                // Prepare the Transfer event
                const transferEvent = prepareEvent({
                    signature: "event Transfer(address indexed from, address indexed to, uint256 value)",
                });

                // Fetch transfer events (last 10000 blocks for performance)
                const transferEvents = await getContractEvents({
                    contract: cusdContract,
                    events: [transferEvent],
                    fromBlock: BigInt(1), // Start from block 1 to avoid "greater than 0" error
                });

                // Filter transfers involving the user (sent or received)
                const userAddress = account.address.toLowerCase();
                const rawCusdTransfers = transferEvents.filter((event: any) => {
                ...
                // Note: Using Date.now() as fallback since we can't easily get block timestamps without extra RPC calls
                // In a production app, we would fetch block details properly
                const cusdTransfers = rawCusdTransfers.map((event: any) => ({
                    ...event,
    ...
    const transactions = useMemo<Transaction[]>(() => {
        ...
        transactionsData.goalWithdrawns?.forEach((withdrawal: any) => {
            allTransactions.push({
                ...
                note: withdrawal.penalty > 0 ? `Early withdrawal (Penalty: ${withdrawal.penalty} cUSD)` : 'Withdrawal',
            });
        });
